...
def parse_bbox(env_var: str, default: str) -> Tuple[int, int, int, int]:
    ...
    if len(parts) != 4:
        raise ValueError(
            f"{env_var} must have exactly 4 comma-separated integers"
        )
    return parts[0], parts[1], parts[2], parts[3]


LEFT_MONITOR_BBOX: Tuple[int, int, int, int] = parse_bbox(
    "LEFT_MONITOR_BBOX", "0,0,2560,1440"
)
RIGHT_MONITOR_BBOX: Tuple[int, int, int, int] = parse_bbox(
    "RIGHT_MONITOR_BBOX", "2560,0,5120,1440"
)

SMART_CAPTURE = os.getenv("SMART_CAPTURE", "False").lower() in (
    "1",
    "true",
    "yes",
)
...
def parse_monitor_bboxes(env_var: str, default: str) -> dict:
...
# MONITOR_BBOXES: mapping hotkeys to screen regions for capture in hotkey mode.
# Format: hotkey:left,top,right,bottom;hotkey2:left,top,right,bottom;...
# Example: ctrl+1:0,0,2560,1440;ctrl+2:2560,0,5120,1440
MONITOR_BBOXES = parse_monitor_bboxes(
