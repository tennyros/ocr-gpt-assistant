...
class TestAnalyzeTextFromImage(unittest.TestCase):
    ...
    def test_empty_text(self, mock_extract_text, mock_ask_ollama, mock_notifier):
        # Check behavior with empty text
        notifier_instance = mock_notifier.return_value
        ocr_processor.analyze_text_from_image("")
        notifier_instance.notify.assert_called_with("OCR Result", "No text was recognized")
        mock_ask_ollama.assert_not_called()

    @patch('core.ocr_processor.Notifier')
    @patch('core.ocr_processor.ask_ollama', side_effect=Exception("Ollama error"))
    @patch('core.ocr_processor.extract_text', return_value="Some text")
    def test_ollama_error(self, mock_extract_text, mock_ask_ollama, mock_notifier):
    ...
    @patch('core.ocr_processor.extract_text', return_value="Some text")
    def test_successful_analysis(self, mock_extract_text, mock_ask_ollama, mock_notifier):
